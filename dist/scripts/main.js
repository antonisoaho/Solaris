...
const createPlanets = (planetData) => {
  planetData === null || planetData === void 0
    ? void 0
...
const infoPlanetStyler = (planetName) => {
  ...
  const computedPlanet = planetElement ? window.getComputedStyle(planetElement) : null;
  const hasBefore = computedPlanet
    ? window.getComputedStyle(planetElement, ':before').getPropertyValue('content') !== 'none'
    : false;
  const backgroundColor =
    computedPlanet === null || computedPlanet === void 0 ? void 0 : computedPlanet.backgroundColor;
  if (backgroundColor) {
